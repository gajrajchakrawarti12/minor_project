...
public class GenerationService {
    ...
    private List<RoomEntity> resolvePracticalRooms(
        ...
        for (RoomEntity room : precomputedSlots.labRooms()) {
            // Common labs: department_id = NULL (available to all batches)
        ...
        if (semester <= 2) {
            ...
            result.addAll(departmentSpecificLabs);
            return result;
        }

        // Higher semesters: Department-specific labs first, then common as fallback
        List<RoomEntity> result = new ArrayList<>(departmentSpecificLabs);
        result.addAll(commonLabs);
        return result;
    }
